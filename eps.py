...
def find_max_steps(epsilon_target, noise_multiplier, q, delta, tolerance=1e-6):
    if epsilon_target < 0:
        return 0
    if epsilon_target == 0:
        return 0  
    ...
    # 无隐私消耗
    if q == 0:
        return float('inf') if epsilon_target >= 0 else 0
    ...
    while True:
        if high > max_initial:
            high = int(max_initial)
            break
        current_epsilon, _, _ = compute_epsilon_alpha(noise_multiplier, high, q, delta)
        if current_epsilon > epsilon_target + tolerance or high >= max_initial:
            break
        high *= 2
    ...
    while low <= high:
        ...
        if mid == 0:
            ...
            current_epsilon, _, _ = compute_epsilon_alpha(noise_multiplier, mid, q, delta)
        
        if current_epsilon <= epsilon_target + tolerance:
            best_steps = mid
            low = mid + 1
        else:
            high = mid - 1
...
if __name__ == '__main__':
    ...
    max_steps = find_max_steps(target_epsilon, noise_multiplier, q, delta)
    print(f"最大通信轮数: {max_steps}")
